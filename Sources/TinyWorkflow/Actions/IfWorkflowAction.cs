...
namespace TinyWorkflow.Actions
{
    internal class IfWorkflowAction<T> : WorkflowAction<T>
    {
        ...
        /// </summary>
        public Workflow<T> WorkflowIfTrue { get; private set; }

        /// <summary>
        /// Action embeded in the step if False
        /// </summary>
        public Workflow<T> WorkflowIfFalse { get; private set; }
        ...
        public IfWorkflowAction(Func<T, bool> condition, Workflow<T> actionIfTrue, Workflow<T> actionIfFalse)
        {
            Condition = condition;
        ...
        public override void Unblock(int unblockLevel)
        {
            if (EvaluatedConditionOnResolve == true)
            {
                if (WorkflowIfTrue != null)
                {
                    WorkflowIfTrue.UnblockInternal(unblockLevel);
                }
            }
            else if (EvaluatedConditionOnResolve == false)
            {
                if (WorkflowIfFalse != null)
                {
                    WorkflowIfFalse.UnblockInternal(unblockLevel);
                }
            }
        }
        #endregion
